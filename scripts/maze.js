...
/**
 * Generate a maze using 3D cube objects as walls
 * @param {number} gridSize - Size of the grid (gridSize x gridSize)
 * @param {number} cellSize - Size of each cell in the grid
 * @returns {Array} Array of Cube objects forming a maze
 */
function generateMazeCubes(gridSize, cellSize) {
  const maze = [];
  const wallHeight = 200;
  const wallWidth = 200;
  ...
  for (let row = 0; row < gridSize; row++) {
    for (let col = 0; col < gridSize; col++) {
      ...
      // Vertical wall on right side (North-South direction)
      if (col < gridSize - 1 && random > 0.6 && !isStart) {
        maze.push(new Cube(x + halfCell, 0, z, 0, 0, 0, wallThickness, wallHeight, wallWidth, texture));
      }

      // Horizontal wall on bottom side (East-West direction)
      if (row < gridSize - 1 && random < 0.4 && !isStart) {
        maze.push(new Cube(x, 0, z + halfCell, 0, 0, 0, wallWidth, wallHeight, wallThickness, texture));
      }
    }
  }
